...
const NUMBER_OF_WINES_PER_BARS = 100;
...
insertMany(barDenormalizedStatement)(
  bars.map(bar => ({ ...bar, winesIds: JSON.stringify(bar.winesIds) }))
);

console.log("==> BAR NORMALIZED INSERTION");
insertMany(barNormalizedStatement)(
  bars.map(bar => {
    const barCopy = { ...bar };
